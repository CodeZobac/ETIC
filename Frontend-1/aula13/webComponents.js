...
const webDetailTemplate = document.createElement("template");
webDetailTemplate.innerHTML = `
<style>
  #detail-container {
    display: flex;
    flex-direction: column;
...
class WebDetail extends HTMLElement {
  ...
  constructor() {
    super();
    this.shadowRoot = this.attachShadow({ mode: "closed" });
    this.shadowRoot.appendChild(webDetailTemplate.content.cloneNode(true));
    this.detailContainer = this.shadowRoot.querySelector("#detail-container");
  }

  async connectedCallback() {
    try {
      const response = await fetch("assets/gallery_data.json");
      const data = await response.json();

      if (data) {
        console.log(data);
        this.detailContainer.querySelector("h1").innerText = data.title;
        this.detailContainer.querySelector("p").innerText = data.description;
      } else {
        console.error("No data found in gallery_data.json");
      }
    } catch (error) {
  ...
  attributeChangedCallback(attrName, oldVal, newVal) {
    if (attrName === "title") {
      this.detailContainer.querySelector("h1").innerText = newVal;
    } else if (attrName === "description") {
      this.detailContainer.querySelector("p").innerText = newVal;
    }
  }
}

customElements.define("web-detail", WebDetail);
